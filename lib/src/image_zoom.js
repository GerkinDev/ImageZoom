...
(function(){
	 ...
	 * @param   {number} [options.minZoom=1] Minimum zoom level done by this instance. Also set the initial zoom
	 * @param   {number} [options.maxZoom=2] Maximum zoom level done by this instance.
	 * @param   {number} [options.zoom=null] Shorthand for { maxZoom: x, minZoom: x}. Will lock zoom
	...
	ImageZoom = function(selector, options){
		...
		this.recalculatePositions = function(){
			...
			if(currentZoom === null){ // Init zoom to minZoom
				var imgZoomDims = {
					width: dims.image.width * self.opts.minZoom,
					height: dims.image.height * self.opts.minZoom
				}
				zoomedImage.style.width = imgZoomDims.width + "px";
				zoomedImage.style.height = imgZoomDims.height + "px";
			}
